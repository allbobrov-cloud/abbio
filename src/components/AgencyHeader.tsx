...
export function AgencyHeader() {
  ...
  return <header className={styles.header} onKeyDown={event => { if (event.key === "Escape" && open) { setOpen(false); toggle.current?.focus(); } }}><div className={styles.headerInner}>
    ...
    <nav id="main-navigation" className={styles.nav} data-open={open} aria-label="Основная навигация">{items.map(([label, href]) => <Link key={href} href={href} aria-current={pathname.startsWith(href) ? "page" : undefined} onClick={() => setOpen(false)}>{label}</Link>)}</nav>
