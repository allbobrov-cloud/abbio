...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; if (slug === "websites") return { title: "Сайты, которые приводят заявки", description: "Разрабатываем сайты для бизнеса: объясняем предложение, ведём посетителя к обращению и передаём заявку в CRM." }; if (slug === "marketing") return { title: "Маркетинг для бизнеса", description: "Связываем спрос, каналы, страницы, обращения и CRM в понятный рабочий маршрут." }; if (slug === "design") return { title: "Дизайн для бизнеса", description: "Визуальная концепция, веб-дизайн и материалы, которые помогают ясно представить продукт." }; const service = services.find(item => item.slug === slug); return { title: service?.title || "Услуга не найдена", description: service?.description }; }
